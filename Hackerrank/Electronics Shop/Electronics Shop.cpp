...
// We only have to complete the below function, rest of the functions will be given by hackerrank itself
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
    sort(keyboards.begin(),keyboards.end());                             // Sorting prices of keyboards and drives
    sort(drives.begin(),drives.end());
    int max = -1;
    for (int i = 0; i < keyboards.size();i++)
    {
        for (int j = 0; j < drives.size();j++)                           // Iterating through every possible pairs of the keyboards and drives
        {
            ...
            else if (cost <= b)                                          // Calculating maximum possible cost
            {
                if (max < cost)
                {
                    max = cost;
