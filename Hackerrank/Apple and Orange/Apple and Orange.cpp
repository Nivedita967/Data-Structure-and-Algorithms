...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    for (int i = 0; i < apples.size();i++)            // Calculating distance where the apple will fall
    {
        apples[i] += a;
    }
    for (int j = 0; j < oranges.size();j++)          // Calculating distance where the orange will fall
    {
        oranges[j] += b;
    }
    int count = 0;
    for (int i = 0; i < apples.size();i++)           // Calculating number of apples that fall on the house
    {
    ...
    cout<<count<<endl;
    count = 0;
    for (int i = 0; i < oranges.size();i++)           // Calculating number of orange that fall on the house
    {
